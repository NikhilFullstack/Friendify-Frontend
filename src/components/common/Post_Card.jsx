            ...
            {/* <!-- component --> */}
                ...
                <div className="bg-white border rounded-sm max-w-md">
                    <div className="flex items-center px-4 py-3">
                        <img className="h-8 w-8 rounded-full" src={post?.userId?.image} />
                        <div className="ml-3 ">
                            <span className="text-sm font-semibold antialiased block leading-tight">{post?.userId?.firstName}<span> </span>{post?.userId?.lastName}</span>
                            <span className="text-gray-600 text-xs block">{post?.caption}</span>
                        </div>
                    </div>
                    {post?.media.map((a,index)=>{
                       return <img src={a} loading='lazy' key={index} />
                    })}
